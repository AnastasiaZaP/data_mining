...
class GbBlogParse:
    ...
    def _get_comments(self, id):
        path = f"api/v2/comments?commentable_type=Post&commentable_id={id}&order=desc"
        response = self._get_response(urljoin(self.start_url, path))
        data = response.json()
        return data

    def _save(self, data):
        collection = self.db["gb_parse"]["gb_parse_comments"]
...
if __name__ == "__main__":
    ...
    #api/v2/comments?commentable_type=Post&commentable_id={id}&order=desc

    #https: // gb.ru / api / v2 / comments?commentable_type = Post & commentable_id = 2628 & order = desc